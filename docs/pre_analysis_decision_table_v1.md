...
| Query و کلیدواژه‌ها | نسخه تحویلی به هر همکار مرجع تاریخی است؛ رشته واقعاً اجراشده از Log/Config ثبت می‌شود | جلوگیری از بازنویسی گذشته و Query bias پنهان | Query نامعلوم `unknown` می‌ماند؛ Query جدید وارد Trend اصلی گذشته نمی‌شود | در انتظار Audit اجرا |
...
| روش Sampling | Collection: غیراحتمالی مبتنی بر Query/Source و نمونه در دسترس محدود به خروجی پلتفرم؛ هدف، نگهداری همه رکوردهای برگشتی بوده است. Gold Sample: تصادفی طبقه‌بندی‌شده | احتمال انتخاب محتوا معلوم نیست؛ این نام‌گذاری دقیق‌ترین توصیف است | تعمیم جمعیتی و Margin of Sampling Error مجاز نیست؛ Composition و Coverage گزارش می‌شود | نوع کلی قطعی؛ اجرای واقعی در انتظار Audit |
...
| مدل و Provider LLM | یک Pipeline واحد با خروجی JSON و Prompt نسخه‌بندی‌شده؛ انتخاب مدل پس از Pilot روی ۱۰۰ رکورد و پیش از Full run قفل می‌شود | انتخاب بدون دیدن زبان، حجم و خطای Pilot قابل دفاع نیست | مقایسه بی‌پایان مدل ممنوع؛ انتخاب با Macro-F1، Failure، Cost و Latency ثبت می‌شود | در انتظار Pilot |
| سقف هزینه و زمان اجرا | Data intake بدون LLM انجام می‌شود. پیش از Full run، سقف عددی هزینه و زمان بر اساس حجم داده و Pilot در Decision Log تأیید می‌شود؛ تا آن زمان Full run مجاز نیست | حجم داده هنوز تحویل نشده و هزینه قابل محاسبه نیست | هزینه کنترل‌نشده؛ با Batch، Cache بر اساس Hash متن و اجرای آزمایشی کنترل می‌شود | نیازمند تأیید تیم پس از تحویل داده |
...
1. رشته Query، Sort، Cap و Pagination واقعاً اجراشده؛
2. تعداد رکورد و Coverage واقعی هر Platform؛
3. امکان اجرای Author-balanced analysis؛
4. اندازه نهایی Gold Sample در صورت کوچک‌بودن یک Platform؛
5. نام دقیق مدل/Provider و Threshold؛
6. سقف عددی هزینه و زمان.
