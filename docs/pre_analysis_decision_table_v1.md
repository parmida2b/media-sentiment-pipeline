...
| Targetهای Stance | Targetهای مجاز `T01` تا `T06` هستند. برای هر متن فقط یک Target اصلی انتخاب می‌شود. `T01` تا `T03` اولویت تحلیل اصلی و `T04` تا `T06` تحلیل‌های تکمیلی‌اند | سه Target نخست مستقیماً با تشدید نظامی، دیپلماسی و فشار اقتصادی مرتبط‌اند؛ سه Target دیگر پوشش موضوعی داده را کامل می‌کنند | افزایش تعداد Targetها هزینه برچسب‌گذاری و نیاز به حجم کافی Gold Sample را افزایش می‌دهد؛ نتایج Targetهای کم‌نمونه توصیفی گزارش می‌شوند | قطعی |
