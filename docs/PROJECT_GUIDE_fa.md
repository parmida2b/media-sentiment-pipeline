...
اگه فقط می‌خوای ببینی پروژه چیکار کرده (بدون اجرای هیچی) — برو سراغ بخش ۳.
اگه می‌خوای از داده خام، Pipeline رو واقعاً از صفر اجرا کنی — اول بخش ۲.۱ زیر رو بخون.

### ۲.۱ دانلود داده خام (`data/raw/` و `data/raw_original/`)

این دو پوشه عمداً روی گیت commit نشدن (حجم بالا + ملاحظات ToS/PII که در بخش ۱۲ توضیح دادم) — بدون این‌ها
فقط می‌تونی از قدم Harmonization به بعد (بخش ۵) اجرا کنی، نه از خودِ داده خام. نسخه‌شون در Google Drive
آپلود شده:

| پوشه مقصد در ریپو | لینک دانلود | حجم تقریبی |
|---|---|---:|
| `data/raw/` | [دانلود](https://drive.google.com/file/d/1HpHRALZWdPGExqBiLn7r_P5QK4DFO55R/view?usp=drive_link) | ~۹۷۰ مگابایت |
| `data/raw_original/` | [دانلود](https://drive.google.com/file/d/173tKJY1gdw-FB0mT6c2kerQR-WKDXH2S/view?usp=sharing) | ~۵۹۸ مگابایت |

بعد از دانلود، فایل‌ها رو Extract کن و دقیقاً همون مسیر (`data/raw/...` و `data/raw_original/...`، کنار
بقیه‌ی زیرپوشه‌های موجود `data/`) قرار بده — مسیر دیگه‌ای بذاری، اسکریپت‌ها پیداشون نمی‌کنن.

⚠️ **این لینک‌ها برای اجرای کد توسط تیم/مشاوره، نه برای انتشار عمومی.** طبق `docs/decision_log.md`
(۲۰۲۶-۰۸-۰۴)، بخشی از داده‌ی قدیمی v1 یوتیوب داخل `data/raw/iran_us_war/` هنوز `author_display_name`
خام (هش‌نشده) دارد. قبل از هر اشتراک‌گذاری وسیع‌تر از این دو لینک، این رکوردها باید remediate بشن — و
تنظیمات دسترسی لینک (`anyone with the link` در برابر لیست محدود) هم متناسب با مخاطب واقعی تنظیم بشه.

**نکته مهم:** دانلود این دو پوشه مشکل بخش ۳ (نوت‌بوک اصلی از نیمه به بعد کار نمی‌کنه) رو حل **نمی‌کنه** —
اون مشکل مربوط به `outputs/tables/` و `annotated_dataset.parquet` است (بخش ۱۲)، نه داده خام. این‌جا فقط
اگه بخوای از صفر (Ingestion → Harmonization → …) اجرا کنی لازمت می‌شه.
